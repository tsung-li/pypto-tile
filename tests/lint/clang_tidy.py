...
def filter_by_diff(
    ...
    changed_headers = [
        f for f in changed if Path(f).suffix.lower() in HEADER_EXTENSIONS
    ]
...
def _detect_nanobind_cmake_dir() -> str | None:
    ...
    try:
        ...
        print(
            f"[clang-tidy] Warning: Could not detect nanobind: {exc}", file=sys.stderr
        )
        return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    _vprint(
        f"[clang-tidy] Collected {len(files)} source file(s), {len(headers)} header file(s)."
    )
